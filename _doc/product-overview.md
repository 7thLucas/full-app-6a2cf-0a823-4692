...
3. **Code execution & self-authored skills (sandboxed runtime + persistent skill library)** — lets Axon write *and run* work, verify its own output, and — critically — *keep* what works. Tools and code Axon authors are saved as permanent, reusable capabilities it can call again, so it extends its own toolkit instead of discarding each run. This is the concrete form of "learning new skills": solve a problem once, own that capability forever. A self-extending toolkit, not disposable sandbox runs.
4. **Memory-conditioned adaptation (already seeded)** — the five persistent logs are the real, buildable form of "self-improvement": Axon gets sharper at *this user's* tasks because it remembers what worked and what upset them. This compounds over time and is the honest substitute for weight retraining. Together, the self-authored skill library (capability 3) and the logs are Axon's two complementary growth mechanisms: skills give it new *hands*, logs make those hands sharper at this user's work.
